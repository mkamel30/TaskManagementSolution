import React, { useState, useMemo } from 'react';
    ...
    const BakeryQuotasPage = () => {
      ...
      const { data: quotas, isLoading, isError } = useQuery<BakeryQuota[]>({
        ...
        onError: (error: any) => { // Add onError callback here
          showError(`خطأ في جلب بيانات المخابز: ${error.message || 'خطأ غير معروف'}`);
        }
      });
      ...
      const { data: historyCounts } = useQuery({
        ...
        queryFn: async () => {
          ...
          if (error) {
            ...
            // Do not throw here, just return empty map, as this is a secondary query
      ...
      // The isError check will now be handled by the onError toast, but keep this for initial render
      if (isError) return <div className="text-center p-8 text-red-500">حدث خطأ أثناء جلب بيانات المخابز. يرجى مراجعة رسالة الخطأ أعلاه.</div>;
